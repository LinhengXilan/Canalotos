...
namespace Memory
{
	...
	{
		static MemoryDescriptorData memoryDescriptorData{};
		void Init(const EFIDataMemory& efiMemory)
		{
			memoryDescriptorData.kernelCodeAddress = 0;
			memoryDescriptorData.kernelDataAddress = 0;
			memoryDescriptorData.memoryPageDescriptor = nullptr;
			...
			memoryDescriptorData.pageCount = PageCount;
			...
			memoryDescriptorData.memoryPageDescriptor = reinterpret_cast<MemoryPageDescriptor*>(startAddress);
			MemoryPageDescriptor* memoryPage = memoryDescriptorData.memoryPageDescriptor;
		...
		{
			return memoryDescriptorData.memoryPageDescriptor;
		}

		MemoryPageDescriptor* GetPageDescriptor(uint32_t index)
		{
			if (index > memoryDescriptorData.pageCount)
			{
				return nullptr;
			}
			return memoryDescriptorData.memoryPageDescriptor + index * sizeof(MemoryPageDescriptor);
		}
	}
}